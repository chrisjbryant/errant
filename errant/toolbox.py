...
def process_m2(
    info_str: str
) -> Tuple[List[str], Dict[str, Tuple[List[str], List[Edit]]]]:
    info = info_str.split("\n")
    ...
    for coder, edits in edit_dict.items():
        ...
        for edit in edits:
            ...
            if edit[2] in {"noop", "Um"}:
                ...
                edit_obj = Edit((edit[0], edit[1]), (-1, -1), edit[3], error_type)
                gold_edits.append(edit_obj)
                continue
            orig_start = edit[0]
            orig_end = edit[1]
            ...
            error_type = ErrorType.from_string(edit[2])
            edit_obj = Edit(
                (orig_start, orig_end), (cor_start, cor_end), edit[3], error_type
            )
            gold_edits.append(edit_obj)
...
def process_edits(edits: List[str]) -> Dict[str, List[Tuple[int, int, str, str]]]:
    edit_dict: Dict[str, List[Tuple[int, int, str, str]]] = {}
    for edit_str in edits:
        ...
        cor = edit[2]
        anntoator_id = edit[-1]
        # Save the useful info as a tuple
        proc_edit = (start, end, cat, cor)
        # Save the proc edit inside the edit_dict using coder id.
        if anntoator_id in edit_dict.keys():
            edit_dict[anntoator_id].append(proc_edit)
        else:
            edit_dict[anntoator_id] = [proc_edit]
    return edit_dict

...
def categorize(
    ...
    # Replacement and special cases
        ...
        # Classify the edit as if the last token weren't there.
            ...
            min_edit.original_span = (
                min_edit.original_span[0],
                min_edit.original_span[1] - 1,
            )
            min_edit.corrected_span = (
                min_edit.corrected_span[0],
                min_edit.corrected_span[1] - 1,
            )
            return categorize(
                min_edit, orig_sent, cor_sent, gb_spell, tag_map, nlp, stemmer
            )
        # Replacement
        else:
            op = ErrorType.REPLACEMENT_OP
            cat = get_two_sided_category(
                orig_toks, cor_toks, gb_spell, tag_map, nlp, stemmer
            )
...
def get_edit_info(
    toks: List[Token], tag_map: Dict[str, str]
...
# Output: An error type string.
def get_two_sided_category(
    orig_toks: List[Token],
    cor_toks: List[Token],
    gb_spell: List[str],
    tag_map: Dict[str, str],
    nlp: Language,
    ...
    if len(orig_str) == len(cor_str) == 1:
        ...
        # Contraction. Rule must come after possessive.
        if (
            orig_str[0].lower() in _CONTS or cor_str[0].lower() in _CONTS
        ) and orig_pos == cor_pos:
            return "CONTR"
        ...
        if (
            ...
            and orig_pos[0] in _OPEN_TAGS
            and cor_pos[0] in _OPEN_TAGS
        ):
            ...
            if orig_pos == cor_pos:
                ...
                if orig_pos[0] == "VERB":
                    ...
                    if orig_toks[0].tag_ in {"VBG", "VBN"} or cor_toks[0].tag_ in {
                        "VBG",
                        "VBN",
                    }:
                        return "VERB:FORM"
        ...
        if (
            stemmer.stem(orig_str[0]) == stemmer.stem(cor_str[0])
            and orig_pos[0] in _OPEN_TAGS
            and cor_pos[0] in _OPEN_TAGS
        ):
            return "MORPH"
        ...
        if set(orig_pos + cor_pos) == {"PART", "PREP"} or set(orig_dep + cor_dep) == {
            "prt",
            "prep",
        }:
            return "PART"
    ...
    if (orig_pos == ["NOUN", "PART"] or cor_pos == ["NOUN", "PART"]) and same_lemma(
        orig_toks[0], cor_toks[0], nlp
    ):
        return "NOUN:POSS"
    # Adjective forms with "most" and "more"; e.g. more free -> freer
    if (
        (
            orig_str[0].lower() in {"most", "more"}
            or cor_str[0].lower() in {"most", "more"}
        )
        and same_lemma(orig_toks[-1], cor_toks[-1], nlp)
        and len(orig_str) <= 2
        and len(cor_str) <= 2
    ):
        return "ADJ:FORM"
...
def same_lemma(orig_tok: Token, cor_tok: Token, nlp: Language) -> bool:
    ...
    for pos in _OPEN_POS:
        ...
        orig_lemmas.append(
            nlp.vocab.morphology.lemmatize(
                pos, orig_tok.lower, nlp.vocab.morphology.tag_map
            )
        )
        cor_lemmas.append(
            nlp.vocab.morphology.lemmatize(
                pos, cor_tok.lower, nlp.vocab.morphology.tag_map
            )
        )

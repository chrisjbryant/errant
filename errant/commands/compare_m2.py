...
class CompareM2(Subcommand):
    def add_subparser(self, name: str, parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
        ...
        subparser.add_argument("-b", "--beta", help="Value of beta in F-score. (default: 0.5)", default=0.5, type=float)
        subparser.add_argument("-v", "--verbose", help="Print verbose output.", action="store_true")
        ...
        subparser.add_argument("-single", help="Only evaluate single token edits; i.e. 0:1, 1:0 or 1:1", action="store_true")
        subparser.add_argument("-multi", help="Only evaluate multi token edits; i.e. 2+:n or n:2+", action="store_true")
        subparser.add_argument("-filt", help="Do not evaluate the specified error types", default=[], nargs="+")
        subparser.add_argument("-cat",	help="Show error category scores.\n"
...
def evaluate_edits(hyp_dict, ref_dict, best, sent_id, args):
    ...
    for hyp_id in hyp_dict.keys():
        for ref_id in ref_dict.keys():
            ...
            p, r, f = compute_f_score(tp+best["tp"], fp+best["fp"], fn+best["fn"], args.beta)
...
def compute_f_score(tp, fp, fn, beta):
...
def print_results(best, best_cats, args):
    ...
    print("\t".join(map(str, [best["tp"], best["fp"], best["fn"]]+list(compute_f_score(best["tp"], best["fp"], best["fn"], args.beta)))))
    print('{:=^46}'.format(""))
    print("")

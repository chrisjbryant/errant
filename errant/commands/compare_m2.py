...
class CompareM2(Subcommand):
    def add_subparser(
        ...
        subparser.add_argument("-hyp", help="A hypothesis M2 file", required=True)
        subparser.add_argument("-ref", help="A reference M2 file", required=True)
        ...
        eval_type = subparser.add_mutually_exclusive_group()
        eval_type.add_argument(
            "-dt", help="Evaluate Detection in terms of Tokens.", action="store_true"
        )
        eval_type.add_argument(
            "-ds", help="Evaluate Detection in terms of Spans.", action="store_true"
        )
        eval_type.add_argument(
            "-cs",
...
def compare_m2_with_args(args: argparse.Namespace) -> None:
    ...
    best_dict: typing.Counter[str] = Counter({"tp": 0, "fp": 0, "fn": 0})
    best_cats: Dict[str, Tuple[int, int, int]] = {}
...
def simplify_edits(sent: str) -> List[Tuple[int, int, str, str, int]]:
    out_edits = []
    ...
    for edit in edits:
        # Preprocessing
        edit_items = edit[2:].split("|||")  # Ignore "A " then split.
        span = edit_items[0].split()
        start = int(span[0])
        end = int(span[1])
        cat = edit_items[1]
        cor = edit_items[2]
        coder = int(edit_items[-1])
...
def process_edits(
    edits: List[Tuple[int, int, str, str, int]], args: argparse.Namespace
) -> Dict[int, Dict[Tuple, List[str]]]:
    coder_dict: Dict[int, Dict[Tuple, List[str]]] = {}
...
def evaluate_edits(hyp_dict, ref_dict, best, sent_id, args):
    ...
    for hyp_id in hyp_dict.keys():
        for ref_id in ref_dict.keys():
            ...
            tp, fp, fn, cat_dict = compare_edits(hyp_dict[hyp_id], ref_dict[ref_id])
            # Compute the local sentence scores (for verbose output only)
            loc_p, loc_r, loc_f = compute_f_score(tp, fp, fn, args.beta)
            # Compute the global sentence scores
            p, r, f = compute_f_score(
            ...
            if (
                ...
                best_tp, best_fp, best_fn, best_f, best_hyp, best_ref = (
                    tp,
                    fp,
                    fn,
                    f,
...
def merge_dict(
    dict1: Dict[str, Tuple[int, int, int]], dict2: Dict[str, Tuple[int, int, int]]
) -> Dict[str, Tuple[int, int, int]]:
    for cat, stats in dict2.items():
        if cat in dict1.keys():
            dict1[cat] = tuple(x + y for x, y in zip(dict1[cat], stats))  # type: ignore
        else:
            dict1[cat] = stats
    return dict1
...
def process_categories(cat_dict, setting):
...
def print_results(best, best_cats, args):
    ...
    # Category Scores
    if args.cat:
        best_cats = process_categories(best_cats, args.cat)
        print("")
        ...
        for cat, cnts in sorted(best_cats.items()):
            cat_p, cat_r, cat_f = compute_f_score(cnts[0], cnts[1], cnts[2], args.beta)
            print(
                cat.ljust(14),
                str(cnts[0]).ljust(8),
                str(cnts[1]).ljust(8),
    ...
    print(
        "\t".join(
            map(
                ...
                + list(compute_f_score(best["tp"], best["fp"], best["fn"], args.beta)),
            )
        )
    )
    print("{:=^46}".format(""))

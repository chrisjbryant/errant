...
from errant.minimum_edits import Opcode, WagnerFischer
...
def get_auto_aligned_edits(
    ...
    # Otherwise, use linguistically enhanced Damerau-Levenshtein
        ...
        minimal_edits = WagnerFischer(
            original_tokens, corrected_tokens, substitution_cost_fn=_token_substitution
        )
...
def _get_edits(
    source: List[Token], target: List[Token], edits: List[Opcode]
) -> List[Opcode]:
    out_edits: List[Opcode] = []
    # Start: Split alignment intro groups of M, T and rest. T has a number after it.
    for op, group in groupby(
        edits, lambda x: x[0][0] if x[0][0] in {"M", "T"} else False
    ):
        # Convert the generator to a list
        op_code_group = list(group)
        # Ignore M
        if op == "M":
            continue
        # Do not merge T
...
def _process_edits(
    ...
    for start, end in combos:
        ...
        if s[-1].tag_ == "POS" or t[-1].tag_ == "POS":
            return (
                _process_edits(source, target, edits[: end - 1])
                + _merge_edits(edits[end - 1 : end + 1])
                + _process_edits(source, target, edits[end + 1 :])
            )
        # Case changes
        if s[-1].lower_ == t[-1].lower_:
            ...
            if start == 0 and (
                (len(s) == 1 and t[0].text[0].isupper())
                or (len(t) == 1 and s[0].text[0].isupper())
            ):
                return _merge_edits(edits[start : end + 1]) + _process_edits(
                    source, target, edits[end + 1 :]
                )
        ...
        if s_str == t_str:
            return (
                _process_edits(source, target, edits[:start])
                + _merge_edits(edits[start : end + 1])
                + _process_edits(source, target, edits[end + 1 :])
            )
        ...
        if end - start < 2:
            ...
            if len(s) == len(t) == 2:
                return _process_edits(
                    source, target, edits[: start + 1]
                ) + _process_edits(source, target, edits[start + 1 :])
            ...
            if end == len(edits) - 1 and (
                (ops[-1] in {"D", "S"} and s[-1].pos == POS.DET)
                or (ops[-1] in {"I", "S"} and t[-1].pos == POS.DET)
            ):
                return _process_edits(source, target, edits[:-1]) + [edits[-1]]
        # Set content word flag
        if not pos_set.isdisjoint(_CONTENT_POS):
            content = True
